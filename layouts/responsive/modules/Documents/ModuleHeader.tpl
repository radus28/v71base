...
<ul class="nav navbar-nav">
					...
					<li class="pull-right">
						...
						{if $MODULE_SETTING_ACTIONS|@count gt 0}
									<button type="button" class="btn app-{$SELECTED_MENU_CATEGORY} module-buttons dropdown-toggle" data-toggle="dropdown">
						...
						{foreach item=BASIC_ACTION from=$MODULE_BASIC_ACTIONS}
							{if $BASIC_ACTION->getLabel() eq 'LBL_ADD_RECORD'}
										<button type="button" class="btn app-{$SELECTED_MENU_CATEGORY} module-buttons dropdown-toggle" data-toggle="dropdown">
											<span class="fa fa-plus" title="{vtranslate('LBL_NEW_DOCUMENT', $MODULE)}"></span>&nbsp;<span class="hidden-sm hidden-xs">{vtranslate('LBL_NEW_DOCUMENT', $MODULE)}</span> &nbsp; <span class="caret"></span>
										</button>
										<ul class="detailViewSetting dropdown-menu">
