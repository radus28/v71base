			...
			{if $MODELNAME == 'ModTracker_Record_Model'}
				...
				{if $PROCEED}
					<div class="row entry clearfix">
						<div class='col-lg-2 col-xs-2 pull-left'>
							...
							{/if}
							<span><i class="ti-{strtolower($VT_ICON)} entryIcon" title={$TRANSLATED_MODULE_NAME}></i></span>&nbsp;&nbsp;
						</div>
					...
					<p class="pull-right muted col-xs-12" style="padding-right:10px;"><span title="{Vtiger_Util_Helper::formatDateTimeIntoDayString("$TIME")}">{Vtiger_Util_Helper::formatDateDiffInStrings("$TIME")}</span></p>
				</div>
			{/if}
			{else if $MODELNAME == 'ModComments_Record_Model'}
				<div class="row">
					<div class="col-lg-2 col-xs-3 pull-left">
						<span><i class="ti-chat entryIcon" title={$TRANSLATED_MODULE_NAME}></i></span>
					</div>
