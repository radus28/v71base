    ...
    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 detailViewButtoncontainer">
        <div class="pull-right btn-toolbar">
            ...
            {if $MODULE_MODEL->isStarredEnabled()}
                <button class="btn btn-se7en markStar {if $STARRED} active {/if}" id="starToggle">
                    <div class='starredStatus' title="{vtranslate('LBL_STARRED', $MODULE)}">
                        <div class='unfollowMessage'>
                            <i class="fa fa-star-o"></i><span class="hidden-xs"> &nbsp;{vtranslate('LBL_UNFOLLOW',$MODULE)}</span>
                        </div>
                        <div class='followMessage'>
                            <i class="fa fa-star active"></i><span class="hidden-xs"> &nbsp;{vtranslate('LBL_FOLLOWING',$MODULE)}</span>
                        </div>
                    </div>
                    <div class='unstarredStatus' title="{vtranslate('LBL_NOT_STARRED', $MODULE)}">
                        <i class="fa fa-star-o"></i><span class="hidden-xs">  {vtranslate('LBL_FOLLOW',$MODULE)}</span>
                    </div>

                </button>
            {/if}
            {foreach item=DETAIL_VIEW_BASIC_LINK from=$DETAILVIEW_LINKS['DETAILVIEWBASIC']}
                <button class="btn btn-se7en" id="{$MODULE_NAME}_detailView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_BASIC_LINK->getLabel())}"
            ...
            {if $DETAILVIEW_LINKS['DETAILVIEW']|@count gt 0}
                <button class="btn btn-se7en dropdown-toggle" data-toggle="dropdown" href="javascript:void(0);">
                   <i class="fa fa-th-list"></i> <span class="hidden-sm hidden-xs">{vtranslate('LBL_MORE', $MODULE_NAME)}</span> &nbsp;&nbsp;<i class="caret"></i>
                </button>
