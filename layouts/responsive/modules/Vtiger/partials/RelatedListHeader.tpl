                    ...
                    {else}
						...
						{if $IS_SELECT_BUTTON || $IS_CREATE_PERMITTED}
							<button type="button" module="{$RELATED_MODULE_NAME}"  class="btn addButton btn-se7en
								...
								{/if}
							{if $IS_SELECT_BUTTON neq true}name="addButton"{/if}>{if $IS_SELECT_BUTTON eq false}<i class="fa fa-plus"></i>&nbsp;{/if}<span class="hidden-sm hidden-xs">&nbsp;{$RELATED_LINK->getLabel()}</span></button>
						{/if}
                    {/if}
                </div>
            {/foreach}&nbsp;
