...
jQuery.Class("Vtiger_RelatedList_Js", {
    ...
    showSearchColumn: function () {
        $('#listview-table').find('.listViewHeaders').find('th').each(function (index, element) {
    ...
    initializeDataTableEvents: function () {
        ...
        $('#listview-table').on('click', 'td.related-list-actions', function () {
...
//            }
    ...
    registerResponsiveSearch: function () {
        ...
        $(document).find('.searchDropDown').on('click','.triggerListSearch',function(){
            $(document).find('.searchRow').find('button[data-trigger="relatedListSearch"]').trigger('click');
        });
    },
...
jQuery(document).ready(function () {
    ...
    instance = Vtiger_RelatedList_Js.getInstance(recordId, moduleName, selectedTabElement, relatedModuleName);

    instance.initializePaginationEvents();
    //For DataTable
    var mediaQuery = window.matchMedia("(max-width: 1000px)");
    registerDataTableEvents(mediaQuery);
    mediaQuery.addListener(registerDataTableEvents.bind(instance));
    //END
});
function registerDataTableEvents(mediaQuery) {
    if (mediaQuery.matches) {
        instance.initializeDataTableEvents();
        instance.registerDropdownSearchOnChange();
        app.event.on('post.listViewFilter.click', function () {
            instance.isResponsiveSearchRegisterd = false;
            instance.initializeDataTableEvents();
            instance.registerDropdownSearchOnChange();
        });
