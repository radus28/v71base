...
Vtiger.Class("Vtiger_List_Js", {
    ...
    registerDataTableEvents: function(){
        alert('test');
    },
...
//			http://stackoverflow.com/questions/26482645/number-isintegerx-which-is-created-can-not-work-in-ie
    ...
    getPageCount: function () {
        ...
        if(pageCountParams.search_value === undefined)
            pageCountParams.search_value="Active";
        var params = {
            "type": "GET",
...
//        }, 10);
       ...
       var mediaQuery = window.matchMedia("(max-width: 1000px)");
        thisInstance.registerDataTableEvents(mediaQuery);
        mediaQuery.addListener(this.registerDataTableEvents.bind(this));
        
        //END
    },
    registerDataTableEvents: function(mediaQuery){
        var thisInstance = this;
        if(mediaQuery.matches){
        thisInstance.initializeDataTableEvents();
        thisInstance.registerDropdownSearchOnChange();
        app.event.on('post.listViewFilter.click', function () {
            thisInstance.isResponsiveSearchRegisterd = false;
            thisInstance.initializeDataTableEvents();
            thisInstance.registerDropdownSearchOnChange();
        });
    }else{
        if(thisInstance.dataTable){
            thisInstance.dataTable.destroy();
            thisInstance.showSearchColumn();
            var searchDiv = $('.searchDropDown');
            searchDiv.hide();
        }
    }
    },
    showSearchColumn: function () {
        $('#listview-table').find('.listViewContentHeader').find('th').each(function (index, element) {
            var style = $(element).attr('style');
            $('#listview-table').find('.searchRow').find('th').eq(index).attr('style', style).show();
        });
    },
    dataTable: false,
    /**
     * @description Initialte & Handle DataTable events.
     * @returns none
     */
    initializeDataTableEvents: function () {
        ...
        var thisInstance = this;
        //Initiate DataTable
        if ($('#listview-table').find('td.listViewRecordActions').length > 0) {
            thisInstance.dataTable = $('#listview-table').DataTable({
                responsive: true,
                keys: false,
                bFilter: false,
                "initComplete": function (settings, json) {
                    thisInstance.hideSearchColumn();
                    thisInstance.showHidedSearchPickList();
                }
            });

            //END
            thisInstance.dataTable.on('responsive-resize', function (e, datatable, columns) {
                thisInstance.hideSearchColumn();
                thisInstance.showHidedSearchPickList();
            });
            // Copy all the <td> css class to datatable <li>
            $('#listview-table').on('click', 'td.listViewRecordActions', function () {
                var tr = $(this).closest('tr');
                var row = thisInstance.dataTable.row(tr);

                if (row.child.isShown()) {
                    var dataId = tr.data('id');
                    var role = tr.attr('role');
                    tr.next('.child').attr('data-id', dataId).attr('role', role);
                    tr.next('.child').find('li').each(function (index, element) {
                        var columnIndex = $(element).attr('data-dt-column');
                        var classNames = tr.find('td').eq(columnIndex).attr('class');
                        var fieldName = tr.find('td').eq(columnIndex).attr('data-name');
                        var title = tr.find('td').eq(columnIndex).attr('title');
                        var rawValue = tr.find('td').eq(columnIndex).attr('data-rawvalue');
                        var fieldType = tr.find('td').eq(columnIndex).attr('data-field-type');
                        $(element).find('.dtr-data').attr('data-dt-column', columnIndex).attr('class', classNames).attr('data-name', fieldName).attr('title', title).attr('data-rawvalue', rawValue).attr('data-field-type', fieldType);
                    });
                } else {
                    // Open this row
                }
    ...
    isResponsiveSearchRegisterd: false,
    showHidedSearchPickList: function () {
        var thisInstance = this;
        var searchDiv = $('.searchDropDown');
        var searchSelectElement = searchDiv.find('.searchoption');

        searchDiv.find('select.searchoption').select2('destroy');
        searchSelectElement.find('option').remove();
        var count = 0;
        $('#listview-table').find('.listViewContentHeader').find('th').each(function (index, element) {
            var style = $(element).attr('style');
            var stylejson = thisInstance.css2json(style);
//            if (stylejson.display === 'none;' || stylejson.display === 'none') {
                if ($('#listview-table').find('.searchRow').find('th').eq(index).find('.select2_search_div').length > 0) {
                    var searchElement = $('#listview-table').find('.searchRow').find('th').eq(index).find('.select2_search_div').find('.listSearchContributor');
                    searchElement = $(searchElement[1]);
                } else {
                    var searchElement = $('#listview-table').find('.searchRow').find('th').eq(index).find('.listSearchContributor');
                }
                if(searchElement.attr('data-fieldinfo') !== undefined){
                var decode = $.parseHTML(searchElement.attr('data-fieldinfo'))[0]['textContent'];
                var fieldInfo = $.parseJSON(decode);
                var label = fieldInfo.label;
                var fieldName = fieldInfo.name;
                var html = '<option value="' + fieldName + '" ';
                html += (count == 0) ? 'selected="selected"' : '';
                html += '>' + label + '</option>';
                searchSelectElement.append(html);
                searchDiv.show();
                if (count === 0) {
                    searchDiv.find('.searchfield').html(searchElement.clone().removeClass('inputElement').removeClass('listSearchContributor').addClass('listSearchContributorClone'));
                    if (searchElement.is('select')) {
                        var values = searchElement.val();
                        if (typeof values === 'object') {
                            $.each(values, function (index, value) {
                                searchDiv.find('.searchfield').find('select.listSearchContributorClone').find('option[value="' + value + '"]').prop('selected', 'selected');
                            });
                        }
                    }
                }
...
//            }
        ...
        if (!thisInstance.isResponsiveSearchRegisterd) {
            thisInstance.registerResponsiveSearch();
            thisInstance.isResponsiveSearchRegisterd = true;
        }
        vtUtils.applyFieldElementsView(searchDiv);
    },
    registerDropdownSearchOnChange: function () {
        var thisInstance = this;
        $(document).find('.searchDropDown').on('change', '.searchoption', function (e) {
            var searchDiv = $('.searchDropDown');
            var fieldName = $(this).val();
            var searchElement = $(document).find('#listview-table').find('.searchRow').find('[name="' + fieldName + '"]');
            searchDiv.find('.searchfield').html(searchElement.clone().removeClass('inputElement').removeClass('listSearchContributor').removeClass('select2-offscreen').addClass('listSearchContributorClone'));
            if (searchElement.is('select')) {
                var values = searchElement.val();
                if (typeof values === 'object') {
                    $.each(values, function (index, value) {
                    ...
                    vtUtils.showSelect2ElementView(searchDiv.find('.searchfield').find('select.listSearchContributorClone'));
                }
            } else {
                searchDiv.find('.searchfield').removeClass('select2-dropdown-open');
                vtUtils.registerEventForDateFields(searchDiv.find('.dateField').not('.ignore-ui-registration'));
                vtUtils.registerEventForTimeFields(searchDiv.find('.timepicker-default'));
            }
            if (!thisInstance.isResponsiveSearchRegisterd) {
                thisInstance.registerResponsiveSearch();
                thisInstance.isResponsiveSearchRegisterd = true;
    ...
    registerResponsiveSearch: function () {
        var searchDiv = $('.searchDropDown');
        $(document).find('.searchDropDown').find('.searchfield').on('change', '.listSearchContributorClone', function (element) {
            var searchDiv = $('.searchDropDown');
            var searchSelectElement = $(searchDiv.find('.searchoption')[1]);
            var fieldName = searchSelectElement.val();
            var targetElement = $(document).find('#listview-table').find('.searchRow').find('[name="' + fieldName + '"]');
            var values = $(this).val();
            if (targetElement.is('select')) {
                targetElement.find('option').removeAttr('selected');
                if (typeof values === 'object') {
                    $.each(values, function (index, value) {
                        targetElement.find('option[value="' + value + '"]').prop('selected', 'selected');
                    });
                }
                vtUtils.showSelect2ElementView(targetElement);
            } else {
                targetElement.val(values);
            }
        });
        
        $(document).find('.searchDropDown').on('click','.triggerListSearch',function(){
            $(document).find('.searchRow').find('button[data-trigger="listSearch"]').trigger('click');
        });
    },
    css2json: function css2json(css) {
        var s = { };
        if (!css)
            return s;
    ...
    /**
     * @description Handle media query changes for DataTable
     * @returns none
     */
    hideSearchColumn: function () {
        $('#listview-table').find('.listViewContentHeader').find('th').each(function (index, element) {
            var style = $(element).attr('style');
            $('#listview-table').find('.searchRow').find('th').eq(index).attr('style', style).hide();
        });
    },
