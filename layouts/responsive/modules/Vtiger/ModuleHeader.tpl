...
{strip}
	<div class="col-sm-12 col-xs-12 module-action-bar clearfix coloredBorderTop">
		<div class="module-action-content clearfix {$MODULE}-module-action-content">
			<div class="col-xs-12 col-lg-7 col-md-7 col-sm-7 module-breadcrumb module-breadcrumb-{$smarty.request.view} transitionsAllHalfSecond">
			...
			<div class="col-xs-12 col-lg-5 col-md-5 col-sm-5 module-breadcrumb-{$smarty.request.view}">
				<div id="appnav" class="navbar-right col-xs-12">
					{if $smarty.request.view eq 'Detail'}
						<div class="btn-group pull-left hidden-md hidden-lg hidden-sm">
							<button class="btn btn-se7en-gray " id="detailViewPreviousRecordButton" {if empty($PREVIOUS_RECORD_URL)} disabled="disabled" {else} onclick="window.location.href = '{$PREVIOUS_RECORD_URL}&app={$SELECTED_MENU_CATEGORY}'" {/if} >
								<i class="fa fa-chevron-left"></i>
							</button>
							<button class="btn btn-se7en-gray  " id="detailViewNextRecordButton"{if empty($NEXT_RECORD_URL)} disabled="disabled" {else} onclick="window.location.href = '{$NEXT_RECORD_URL}&app={$SELECTED_MENU_CATEGORY}'" {/if}>
					...
					<div class="btn-group pull-right">
						...
						{foreach item=BASIC_ACTION from=$MODULE_BASIC_ACTIONS}
							{if $BASIC_ACTION->getLabel() == 'LBL_IMPORT'}
									<button id="{$MODULE}_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($BASIC_ACTION->getLabel())}" type="button" class="btn addButton btn-default app-{$SELECTED_MENU_CATEGORY} module-buttons" 
							...
							{else}
									<button id="{$MODULE}_listView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($BASIC_ACTION->getLabel())}" type="button" class="btn addButton app-{$SELECTED_MENU_CATEGORY} module-buttons" 
											{if stripos($BASIC_ACTION->getUrl(), 'javascript:')===0}  
												onclick='{$BASIC_ACTION->getUrl()|substr:strlen("javascript:")};'
											{else} 
						...
						{if $MODULE_SETTING_ACTIONS|@count gt 0}
									<button type="button" class="btn app-{$SELECTED_MENU_CATEGORY} module-buttons dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
										<span class="fa fa-wrench" aria-hidden="true" title="{vtranslate('LBL_SETTINGS', $MODULE)}"></span><span class="hidden-xs">&nbsp;{vtranslate('LBL_CUSTOMIZE', 'Reports')}&nbsp; </span><span class="caret"></span>
									</button>
									<ul class="detailViewSetting dropdown-menu pull-right">
