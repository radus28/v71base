				...
				{foreach item=SUMMARY_CATEGORY from=$SUMMARY_INFORMATION}
					<div class="row textAlignCenter roundedCorners">
						{foreach key=FIELD_NAME item=FIELD_VALUE from=$SUMMARY_CATEGORY}
							<div class="col-lg-3">
								<div class="well" style="min-height: 100px; text-align: left;border-left: 3px solid #5ea9dd; padding-left: 10px;margin-bottom: 10px;">
									<div>
					...
					<div class="widget_header clearfix">
						...
						{if $DOCUMENT_WIDGET_MODEL->get('action')}
							...
							<div class="pull-right">
								<div class="dropdown">
									<button type="button" class="btn btn-se7en dropdown-toggle" data-toggle="dropdown">
										<span class="fa fa-plus" title="{vtranslate('LBL_NEW_DOCUMENT', $MODULE_NAME)}"></span>&nbsp;<span class="hidden-sm hidden-xs">{vtranslate('LBL_NEW_DOCUMENT', 'Documents')}</span>&nbsp; <span class="caret"></span>
									</button>
				...
				<div class="widgetContainer_troubleTickets" data-url="{$HELPDESK_WIDGET_MODEL->getUrl()}" data-name="{$HELPDESK_WIDGET_MODEL->getLabel()}">
					<div class="widget_header clearfix">
						...
						{if $HELPDESK_WIDGET_MODEL->get('action')}
							<div class="pull-right">
								<button class="btn addButton btn-se7en btn-sm createRecord" type="button" data-url="{$HELPDESK_WIDGET_MODEL->get('actionURL')}">
									<i class="fa fa-plus"></i>&nbsp;<span class="hidden-sm hidden-xs">{vtranslate('LBL_ADD',$MODULE_NAME)}</span>
								</button>
							</div>
						{/if}
					</div>
				...
				<div class="widgetContainer_mileStone" data-url="{$MILESTONE_WIDGET_MODEL->getUrl()}" data-name="{$MILESTONE_WIDGET_MODEL->getLabel()}">
					<div class="widget_header clearfix">
						...
						{if $MILESTONE_WIDGET_MODEL->get('action')}
							<div class="pull-right">
								<button class="btn addButton btn-sm btn-se7en createRecord"  id="createProjectMileStone" type="button" data-url="{$MILESTONE_WIDGET_MODEL->get('actionURL')}">
									<i class="fa fa-plus"></i>&nbsp;<span class="hidden-sm hidden-xs">{vtranslate('LBL_ADD',$MODULE_NAME)}</span>
								</button>
							</div>
						{/if}
				...
				<div class="widgetContainer_tasks" data-url="{$TASKS_WIDGET_MODEL->getUrl()}" data-name="{$TASKS_WIDGET_MODEL->getLabel()}">
					<div class="widget_header clearfix">
						...
						{if $TASKS_WIDGET_MODEL->get('action')}
							<div class="pull-right">
								<button class="btn addButton btn-sm btn-se7en createRecord" id="createProjectTask" type="button" data-url="{$TASKS_WIDGET_MODEL->get('actionURL')}">
									<i class="fa fa-plus"></i>&nbsp;<span class="hidden-sm hidden-xs">{vtranslate('LBL_ADD',$MODULE_NAME)}</span>
								</button>
							</div>
						{/if}
