					...
					<div class="btn-group">
						...
						{foreach item=BASIC_ACTION from=$MODULE_BASIC_ACTIONS}
							{if $BASIC_ACTION->getLabel() == 'LBL_IMPORT'}
									<button id="{$MODULE}_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($BASIC_ACTION->getLabel())}" type="button" class="btn addButton app-{$SELECTED_MENU_CATEGORY} module-buttons" 
						...
						{if $MODULE_SETTING_ACTIONS|@count gt 0}
									<button type="button" class="btn app-{$SELECTED_MENU_CATEGORY} module-buttons dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
										<span class="fa fa-wrench" aria-hidden="true" title="{vtranslate('LBL_SETTINGS', $MODULE)}"></span>&nbsp;<span class="hidden-sm hidden-xs">{vtranslate('LBL_CUSTOMIZE', 'Reports')}&nbsp;</span> <span class="caret"></span>
									</button>
