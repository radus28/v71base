    ...
    <div class='col-lg-12 col-xs-12 padding0px'>
        ...
        <span class="col-lg-5 padding0px">
            <span class="fa-stack fa-sm cursorPointer mmActionIcon" id="mmMarkAsRead" data-folder="{$FOLDER->name()}" title="{vtranslate('LBL_MARK_AS_READ', $MODULE)}">
                <img src="layouts/responsive/skins/images/envelope-open.png" id="mmEnvelopeOpenIcon">
            </span>
        ...
        <span class="col-lg-6 padding0px">
			...
			{if $FOLDER->mails()}<span class="pageInfo">{$FOLDER->pageInfo()}&nbsp;&nbsp;</span> <span class="pageInfoData" data-start="{$FOLDER->getStartCount()}" data-end="{$FOLDER->getEndCount()}" data-total="{$FOLDER->count()}" data-label-of="{vtranslate('LBL_OF')}"></span>{/if}
                <button type="button" id="PreviousPageButton" class="btn btn-se7en-gray marginRight0px" {if $FOLDER->hasPrevPage()}data-folder='{$FOLDER->name()}' data-page='{$FOLDER->pageCurrent(-1)}'{else}disabled="disabled"{/if}>
                    <i class="fa fa-caret-left"></i>
                </button>
                <button type="button" id="NextPageButton" class="btn btn-se7en-gray" {if $FOLDER->hasNextPage()}data-folder='{$FOLDER->name()}' data-page='{$FOLDER->pageCurrent(1)}'{else}disabled="disabled"{/if}>
        ...
        <div class="col-lg-10 col-md-6 col-sm-6 col-xs-12 mmSearchContainerOther">
            <div>
                ...
                <div class="col-lg-4  col-xs-4 padding0px mmSearchDropDown">
                    <select id="searchType" style="background: #DDDDDD url('layouts/responsive/skins/images/arrowdown.png') no-repeat 95% 40%; padding-left: 9px;">
